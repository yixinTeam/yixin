...
export default {
    ...
    methods:{
        ...
        handlereset(index, row,e){
            this.message=[];
            this.$ajax.post('https://10.240.80.72:8443/icc-interface/new/account/batchUpdateSeat',[{'id':row.id,'password':'reset','shortName':row.shortName,'loginName':row.loginName}])
            .then( (res) => {
        ...
        selectreset:function(){
            if(this.multipleSelection.length>0){
                ...
                this.$ajax.post('https://10.240.80.72:8443/icc-interface/new/account/batchUpdateSeat',data)
                .then( (res) => {
                    this.message=res.data.info;
                    this.dialog_type=1;
                    this.checkbox=true;
        ...
        seat_init(data){
            this.$ajax.post('https://10.240.80.72:8443/icc-interface/new/account/findSeatList',data)
            .then( (res) => {
        ...
        findSeat:function(){
            var data={
                'state':this.worker_state,'startTime':this.search_date!=null?this.search_date[0]:"",'endTime':this.search_date!=null?this.search_date[1]:"",'requireTotalCount':true
            };
            for (let key in data){
                if(data[key]==''){
                    delete data[key];
        ...
        sort_change({column, prop, order} ){
            ...
            var data={'state':this.worker_state,'startTime':this.search_date!=null?this.search_date[0]:"",'endTime':this.search_date!=null?this.search_date[1]:"",'requireTotalCount':true,'pageNum':this.pageNum,"orderWay":this.orderWay,'orderField':this.orderField};
            for (let key in data){
                if(data[key]==''){
                    delete data[key];
                }
        ...
        upSeat:function(index,row,item){
            ...
            this.$ajax.post('https://10.240.80.72:8443/icc-interface/new/account/updateSeat',data)
            .then( (res) => {
                if(res.code==200){
                    window.reload()
                }
        ...
        state_select(row,state){
            this.$ajax.post('https://10.240.80.72:8443/icc-interface/new/account/updateSeat',{'id':row.id,'state':state})
            .then( (res) => {
                if(res.code==200){
                    this.reload()
                }
