<template>
    <div class="container">
        <div class="part1">
            ...
            <el-table :data="tableData" style="width: 100%" :default-sort = "{prop: 'date', order: 'descending'}" class="table" @selection-change="handleSelectionChange">
                ...
                <el-table-column prop="p_caozuo" class-name="line11" label="操作"  min-width="160">
                    <template slot-scope="scope">
                        <el-button
                        size="mini" type="text" 
                        @click="handlefp(scope.$index, scope.row)">重置密码</el-button>&#12288;|
            ...
            <Dialog :message='message' :type='dialog_type' :show="dialog_show"></Dialog>
        </div>
    </div>
</template>
<style scoped>
...
export default {
    ...
    methods:{
        ...
        handlecopy(index, row,e){
            this.message=[row.accountPid,row.fullName,row.passowrd];
            this.dialog_type=0;
